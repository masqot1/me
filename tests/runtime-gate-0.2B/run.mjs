...
try {
  ...
  await page.goto(`chrome-extension://${extensionId}/runtime-gate.html`, { waitUntil: 'domcontentloaded', timeout: 30000 });
  ...
  console.log(result);
  if (!result.startsWith('PASS')) throw new Error(`Extension runtime gate failed: ${result}`);
} finally {
  await browser.close();
}
