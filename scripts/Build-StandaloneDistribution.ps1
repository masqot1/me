...
foreach($file in @('Install-TrueWebsiteCloner.ps1','Uninstall-TrueWebsiteCloner.ps1','01_INSTALL.bat','02_RUN_TRUEWEBSITECLONER.bat','03_UNINSTALL.bat','README.txt')){Copy-Item (Join-Path $Root "distribution-standalone\$file") (Join-Path $Stage $file) -Force}
Copy-Item (Join-Path $Root 'VERSION') (Join-Path $Stage 'VERSION') -Force
$records=@();Get-ChildItem $Stage -Recurse -File|Sort-Object{[IO.Path]::GetRelativePath($Stage,$_.FullName).Replace('\','/')}|ForEach-Object{$rel=[IO.Path]::GetRelativePath($Stage,$_.FullName).Replace('\','/');if($rel-ne'distribution-manifest.json'){$records+=[ordered]@{path=$rel;byteLength=[int64]$_.Length;sha256=(Get-FileHash $_.FullName -Algorithm SHA256).Hash.ToLowerInvariant()}}}
...
Add-Type -AssemblyName System.IO.Compression;$fixed=[DateTimeOffset]::new(2000,1,1,0,0,0,[TimeSpan]::Zero);$stream=[IO.File]::Open($ZipPath,[IO.FileMode]::CreateNew,[IO.FileAccess]::ReadWrite,[IO.FileShare]::None)
