...
Run-Step 'Foundation static gate' {
  & (Join-Path $Root 'tests\Test-Foundation.ps1')
  $foundationSucceeded = $?
  if(-not $foundationSucceeded){throw 'Foundation static gate failed.'}
}
Run-Step 'Extension manifest and JavaScript syntax' {
  ...
  foreach($js in Get-ChildItem (Join-Path $Root 'chrome-extension') -Filter '*.js' -File){
    & node --check $js.FullName
    if($LASTEXITCODE-ne 0){throw "JavaScript syntax failed: $($js.Name)"}
  }
}
...
foreach($project in $gateProjects){
  ...
  Run-Step "Gate test: $($project.BaseName)" {
    & dotnet run --project $project.FullName -c Release
    if($LASTEXITCODE-ne 0){throw "$relative exited $LASTEXITCODE"}
  }
}
