...
Write-Host "TrueWebsiteCloner v1.0 - development install" -ForegroundColor Cyan
$dotnet = Get-Command dotnet -ErrorAction SilentlyContinue
if (-not $dotnet) { throw ".NET SDK was not found. Install .NET 10 SDK first." }
$version = (& dotnet --version).Trim(); if (-not $version.StartsWith("10.")) { throw ".NET 10 SDK is required. Detected: $version" }
if (Test-Path $Artifacts) { Remove-Item $Artifacts -Recurse -Force }; New-Item -ItemType Directory -Path $Artifacts | Out-Null
$projects = @(
...
@{Name='Release Tool';Project='src\TrueWebsiteCloner.ReleaseTool\TrueWebsiteCloner.ReleaseTool.csproj';Output='release-tool'},
@{Name='Seal Tool';Project='src\TrueWebsiteCloner.SealTool\TrueWebsiteCloner.SealTool.csproj';Output='seal-tool'},
@{Name='Bundle Tool';Project='src\TrueWebsiteCloner.BundleTool\TrueWebsiteCloner.BundleTool.csproj';Output='bundle-tool'} )
foreach($item in $projects){ & dotnet publish (Join-Path $Root $item.Project) -c Release -r win-x64 --self-contained false -o (Join-Path $Artifacts $item.Output); if($LASTEXITCODE-ne 0){throw "$($item.Name) publish failed."} }
$NativeDir=Join-Path $env:LOCALAPPDATA "TrueWebsiteCloner\native-host";New-Item -ItemType Directory -Path $NativeDir -Force|Out-Null
$ManifestPath=Join-Path $NativeDir "$HostName.json";$HostExe=Join-Path $Artifacts "native-host\TrueWebsiteCloner.NativeHost.exe"
...
$RegPath="HKCU:\Software\Google\Chrome\NativeMessagingHosts\$HostName";New-Item -Path $RegPath -Force|Out-Null;Set-Item -Path $RegPath -Value $ManifestPath
Write-Host "PASS: development binaries published" -ForegroundColor Green;Write-Host "PASS: native host registered for current user" -ForegroundColor Green
